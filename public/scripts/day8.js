 ...
 */
export function solveD8P1(puzzleText) {
    ...
    for (let puzzle of puzzleArray) {
        ...
        for (let i = 11; i < puzzleElements.length; i++) {
            ...
            if (elLen === 2 || elLen === 4 || elLen === 3 || elLen === 7)
                uniqueNumberOfSegmentsDigitsCounter++;
        }
    }
 ...
 * @returns The solution
 */
export function solveD8P2(puzzleText) {
    const puzzleArray = puzzleInputToStringArray(puzzleText);
    let outputSum = 0;
    for (let puzzle of puzzleArray) {
 ...
 */
function prepareDisplayBasedOnDigits(puzzle) {
 ...
 */
function getOutputValue(puzzle, digitDisplay) {
    let puzzleElements = puzzle.trim().split(" ");
    let outputValue = "";
    for (let i = 11; i < puzzleElements.length; i++) {
...
class DigitDisplay {
    ...
    decodeDigit(digit) {
        if (digit.length === 2)
            return "1"; // 1 has 2 segments (c,f) - unique
        else if (digit.length === 3)
            return "7"; // 7 has 3 segments (a,c,f) - unique
        else if (digit.length === 4)
            return "4"; // 4 has 4 segments (b,c,d,f) - unique
            ...
            return "8"; // 8 has 7 segments (a,b,c,d,e,f,g) - unique
        return "";
    }
    /**
     ...
     */
    getDigitsSegmentsSorted(segmentsIndexes) {
        let segmentsOfDigit = new Array();
    ...
    matchDigitToSegments(digit) {
        ...
        if (digit.length === 2) {
            ...
            if (digitsDic.length === 2) {
                ...
                console.log("something went wrong with digitsDic:" + digitsDic + " for digit:" + digit);
        }
        else if (digit.length === 6) {
            // 0 has 6 segments (a,b,c,e,f,g)
            // 6 has 6 segments (a,b,d,e,f,g)
